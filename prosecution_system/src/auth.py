...
def _render_profile_page(user, favorites, history):
    return f"""
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        <div class="card">
            ...
            <div class="info">
                <div class="info-label">收藏数量</div>
                <div class="info-value">{len(favorites)}</div>
            </div>
...
def _render_favorites_page(user, favorites):
    return f"""
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        <div class="card">
            <h1>⭐ 我的收藏 ({len(favorites)})</h1>
            {"".join(f'<div class="item"><a href="/case/{f["case_id"]}">{f["case_id"]}</a></div>' for f in favorites)}
            <a href="/auth/profile" class="btn">返回个人中心</a>
        </div>
    </div>
</body>
</html>
