...
功能：
- 为每条法律结论生成置信度评分（0-100）
- 标注不确定性来源，防止系统给出误导性推断（幻觉缓解）

置信度等级：
  HIGH        (80-100)  法条原文直接匹配，无歧义，可直接引用
  MEDIUM      (50-79)   法条+司法解释推断，逻辑合理，存在其他解释可能
  LOW         (20-49)   类比推理，存在多种解释，引用时必须标注不确定性
  UNRELIABLE  (0-19)    无法确定，必须人工核查

评估维度：
  text_match   文本匹配度   权重 35%
  currency     时效性       权重 25%
  source_count 来源数量     权重 15%
  semantic     语义一致性   权重 15%
  regional     地域差异     权重 10%
"""
...
# 司法解释时效状态 (ACTIVE|SUPERSEDED|REPEALED|UNKNOWN)
...
class ConfidenceScorer:
    ...
    def assess(
        ...
        matched_interpretations: List[str] = None,
        is_direct_quote: bool = False,
        province: str = None,
        crime_type: str = None,
    ) -> ConfidenceScore:
        matched_statutes = matched_statutes or []
        matched_interpretations = matched_interpretations or []
        ...
        score = self._compute(dm)
        reasons = self._reasons(dm, matched_interpretations, province, crime_type)
        lv = _level(score)

        return ConfidenceScore(
            score=score, level=lv,
            label=ConfidenceLevel.LABELS[lv],
            description=ConfidenceLevel.DESCRIPTIONS[lv],
            dimensions=dm, uncertainty_reasons=reasons,
            source_statutes=matched_statutes,
    ...
    def assess_batch(self, conclusions: List[Dict[str, Any]]) -> ConfidenceReport:
        ...
        for i, c in enumerate(conclusions):
            cs = self.assess(
                conclusion=c.get("text", ""),
                matched_statutes=c.get("matched_statutes", []),
        ...
        return ConfidenceReport(total_conclusions=n, high=h, medium=m, low=l,
                                unreliable=u, average_score=avg,
                                conclusions=results, overall_recommendation=rec)
    ...
    def _status(self, interp: str) -> str:
        ...
        for key, (st, *_) in INTERPRETATION_STATUS.items():
            if key in interp or interp in key:
                self._cache[interp] = st
                return st
        self._cache[interp] = "UNKNOWN"
        return "UNKNOWN"
    ...
    def _t_semantic(self, c: str, ct: str = None) -> int:
        amb = ["可能构成", "疑似", "或许", "有待核实", "原则上", "可参照",
               "一般认为", "各地不一", "存在争议", "尚有争议"]
        crit = ["构成", "属于", "应当", "必须", "认定为"]
        ac = sum(1 for p in amb if p in c)
        cc = sum(1 for p in crit if p in c)
        if ac >= 2: return 30
        if cc >= 2: return 40
        if ac == 1: return 55
        if cc >= 1: return 65
    ...
    def _t_regional(self, c: str, prov: str = None, ct: str = None) -> int:
        sensitive = ["盗窃罪", "诈骗罪", "抢夺罪", "敲诈勒索罪",
                     "开设赌场罪", "组织卖淫罪", "强迫卖淫罪"]
        has_p = bool(prov)
        needs_p = ct in sensitive if ct else False
    ...
    def _compute(self, dm: Dict[str, int]) -> int:
        w = {"text_match": 0.35, "currency": 0.25, "source_count": 0.15,
             "semantic": 0.15, "regional": 0.10}
        wtd = sum(dm.get(k, 50) * v for k, v in w.items())
        mn = min(dm.values()) if dm else 50
        return int(wtd * 0.7 + mn * 0.3)

    def _reasons(self, dm: Dict[str, int], itprs: List[str],
                 prov: str, ct: str) -> List[str]:
        r = []
...
def main():
    ...
    parser.add_argument("--crime", type=str)
    ...
    if args.batch:
        ...
        print(json.dumps({
            "total": r.total_conclusions, "high": r.high, "medium": r.medium,
    ...
    dim_labels = {"text_match": "文本匹配度", "currency": "时效性",
                  "source_count": "来源数量", "semantic": "语义一致性", "regional": "地域差异"}
    print(f"\n置信度: {cs.score}/100 [{cs.label}]  {cs.description}")
    if cs.uncertainty_reasons:
        print("不确定性原因: " + "; ".join(cs.uncertainty_reasons))
